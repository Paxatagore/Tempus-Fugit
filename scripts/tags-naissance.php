...
$q = "SELECT DISTINCT lienpl.num, lienpl.lieu, personne.naissance_evenementId
FROM personne, lienpl
WHERE personne.num = lienpl.personne
AND personne.naissance_evenementId > 0
AND (lienpl.num) NOT
IN (SELECT lienpl.num 
FROM evenement, personne, lienpl, lienel
WHERE lienel.evenement = evenement.num 
AND evenement.num = personne.naissance_evenementId
